...
private:
    ...
    void load_parameters()
    {
        ...
        this->declare_parameter<double>("current_lsb", 0.0003);
    ...
    void create_publishers()
    {
        ...
        for (size_t d = 0; d < i2c_addrs_.size(); ++d)
        {
            ...
            for (int ch = 0; ch < 4; ++ch)
            {
                ...
                pubs_[d][ch] = this->create_publisher<std_msgs::msg::Float64>(topic, 10);
            }
        }
    }
        ...
        for (size_t d = 0; d < i2c_addrs_.size(); ++d)
        {
            ...
            for (int ch = 0; ch < 4; ++ch)
            {
                ...
                if (read_reg16(addr, kRegCurrent[ch], raw))
                {
                    ...
                    msg->data = current_a;
                    pubs_[d][ch]->publish(std::move(msg));
                }
            }
    ...
    std::vector<std::vector<rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr>> pubs_;
};
